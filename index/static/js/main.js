...
$(".drink.product_button").click(function () {
  ...
  let product = ALL_PRODUCTS.filter(
    (item) => item.name == `${product_name}` && item.size == `${abbrv_size}`
  );
  let product_index = ALL_PRODUCTS.findIndex(
    (item) => item.name == `${product_name}` && item.size == `${abbrv_size}`
  );
...
$(document).on("click", ".add_button", function () {
  ...
  let product = ALL_PRODUCTS.filter(
    (item) =>
      item.abbr_name == `${product_name}` && item.size == `${product_size}`
  );
  let product_index = ALL_PRODUCTS.findIndex(
    (item) =>
      item.abbr_name == `${product_name}` && item.size == `${product_size}`
...
function update_basket() {
      ...
      `<div class="row product_row"> 
                ...
                <div class="col-1" id="size">
                    <p>${
                      this.size == ""
                        ? ""
                        : this.size[0].toUpperCase() + this.size.substring(1)
                    }</p>
                </div>
  ...
  if (DISCOUNTS.length != 0) {
    $(DISCOUNTS).each(function (index, item) {
      $(".products_rows_div").prepend(
        `<div class="row product_row ${
          item["name"] == "Invalid" ? "invalid" : "valid"
        }">
                    <div class="col-4">
                    ...
                    <div class="col-1" id="delete_row_div">
                        <div class="delete_button basket_edit_button" data-special="${
